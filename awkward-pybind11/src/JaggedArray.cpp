...
Note:			I modeled this function after
				_offsets2parents_fill() in the jagged
...
PYBIND11_MODULE(JaggedArray, m) {
	...
	m.def("offsets2parents", &offsets2parents, 
		"Populates a properly-sized parents array based on the values from an offsets array.");
	m.def("toString", &toString,
		"Converts an array into a string in the format \'{ 1, 2, 3 }\'");
	m.def("showOffsets", &showOffsets,
		"Calls toString() for the offsets array so it can be displayed in the python shell");
	m.def("showParents", &showParents,
		"Calls toString() for the parents array so it can be displayed in the python shell");
}