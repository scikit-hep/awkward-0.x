...
Some awkward-array structures differ from each other in ways that are intentionally not reflected by high-level type. Their purpose is to deal with awkward data sources.

The following purely low-level features 





The high-level types intentionally hide certain features. The most prominent of these are non-contiguousness, which presents a sequence of arrays as though they were concatenated, and laziness, which presents an unloaded or ungenerated array as though it were already materialized. These two features deal with memory-location awkwardness and load-time awkwardness but not type manipulation. These are *purely low-level* features.

Some features modify the type but also have a low-level component. These are:

   * `JaggedArray`, which indexes content by element index, and `ByteJaggedArray`, which indexes content by bytes. Both present a jagged array type.
   * `MaskedArray`, which uses 1-byte booleans as a mask, and `BitMaskedArray`, which uses 1-bit booleans as a mask. `IndexedMaskedArray` uses negative indexes as a mask; the positive indexes 
