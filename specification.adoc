...
Documentation is not an explicit goal of this specification, though users may read it to learn how to use the library. Unlike documentation, however, this document describes what an awkward-array library should do, not necessarily what it does. Users are encouraged to file https://github.com/scikit-hep/awkward-array/issues[issues] if a discrepancy is found.

The major and minor version numbers of this specification, `xx.yy`, correspond to the major and minor version numbers of the Numpy-only awkward-array library: `xx.yy` in `xx.yy.zz`. Its revision number, `zz`, increases as it strives toward compliance with the specification.
