...
It is not a goal of this document to explain how these features are implemented. Drop-in replacements for the Numpy-only awkward-array library and libraries for other environments are free to implement the specification differently. They will thus have different performance characteristics.

Documentation is not an explicit goal of this specification, but it may be used as such.
